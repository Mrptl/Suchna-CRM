...
const initialData = {
  ...
  staffUsers: [
    { id: 4, name: "John Kumar", email: "sales.john@suchna.com", password: "staff123", role: "Sales Executive", department: "Sales", status: "Active", avatar: "JK", assigned_clients: [1, 2], manager_id: 2, assigned_projects: [] },
    { id: 5, name: "Sarah Designer", email: "designer.sarah@suchna.com", password: "staff123", role: "Designer", department: "Projects", status: "Active", avatar: "SD", assigned_clients: [3], manager_id: 3, assigned_projects: [3] },
    { id: 6, name: "Rahul Dev", email: "project.lead@suchna.com", password: "staff123", role: "Developer", department: "Projects", status: "Active", avatar: "RD", assigned_clients: [1], manager_id: 3, assigned_projects: [1] },
    { id: 7, name: "Marketing Team", email: "marketing.team@suchna.com", password: "staff123", role: "Digital Marketing Specialist", department: "Marketing", status: "Active", avatar: "MT", assigned_clients: [1, 2], manager_id: null, assigned_projects: [] },
    { id: 8, name: "Support Team", email: "support.team@suchna.com", password: "staff123", role: "Support Specialist", department: "Support", status: "Active", avatar: "ST", assigned_clients: [1, 2, 3], manager_id: null, assigned_projects: [] },
    { id: 9, name: "Sales Manager", email: "sales.manager@suchna.com", password: "staff123", role: "Sales Manager", department: "Sales", status: "Active", avatar: "SM", assigned_clients: [1, 2, 3], manager_id: null, assigned_projects: [] }
  ],
  clientUsers: [
...
function AppProvider({ children }) {
  ...
  const login = (email, password, type) => {
    if (type === 'admin') {
      ...
      const staff = data.staffUsers.find(u => u.email === email && u.password === password);
      if (staff) {
      ...
      <div className="login-container">
        <div className="login-card">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <button className="btn btn-secondary" onClick={() => setLoginMode('staff')}>
              Staff Login
            </button>
    ...
    <div className="login-container">
      <div className="login-card">
        ...
        <div style={{ marginTop: '16px', fontSize: '12px', color: 'var(--color-text-secondary)', textAlign: 'center' }}>
          {loginMode === 'admin' ? 'Demo: admin@suchna.com / admin123' : loginMode === 'staff' ? 'Demo: sales.john@suchna.com / staff123' : 'Demo: client1@techstart.com / client123'}
        </div>
...
function StaffDashboard() {
  const { data, currentUser } = useApp();
  const assignedClients = data.companies.filter(c => currentUser.assigned_clients?.includes(c.id));
  const assignedProjects = data.projects.filter(p => currentUser.assigned_projects?.includes(p.id) || p.assigned_to_id === currentUser.id);
  const myTasks = data.tasks.filter(t => t.assignee_id === currentUser.id);
  ...
  return (
    <div>
      <div style={{ background: 'var(--color-bg-1)', padding: 'var(--space-20)', borderRadius: 'var(--radius-lg)', marginBottom: 'var(--space-24)' }}>
        <h2 style={{ marginBottom: 'var(--space-8)' }}>Welcome, {currentUser.name}!</h2>
        <p style={{ color: 'var(--color-text-secondary)' }}>{currentUser.role} - {currentUser.department} Department</p>
      </div>

      <div className="dashboard-grid">
        <div className="stat-card">
          <div className="stat-card-title">My Active Tasks</div>
          <div className="stat-card-value">{pendingTasks.length}</div>
          <div className="stat-card-change">{myTasks.length} total tasks</div>
        </div>
        <div className="stat-card">
          <div className="stat-card-title">Tasks Completed</div>
          <div className="stat-card-value">{completedTasks.length}</div>
          <div className="stat-card-change positive">{myTasks.length > 0 ? ((completedTasks.length / myTasks.length) * 100).toFixed(0) : 0}% completion rate</div>
        </div>
        <div className="stat-card">
          <div className="stat-card-title">Assigned Clients</div>
          <div className="stat-card-value">{assignedClients.length}</div>
        </div>
        <div className="stat-card">
          <div className="stat-card-title">Active Projects</div>
          <div className="stat-card-value">{assignedProjects.length}</div>
        </div>
      </div>
      ...
      <div className="grid-2">
        <div className="card">
          <div className="card-header">
            <h3 className="card-title">My Assigned Clients</h3>
          </div>
          <div className="card-body">
            {assignedClients.length > 0 ? (
              ...
              <div className="empty-state"><p>No clients assigned</p></div>
            )}
          </div>
        </div>
          ...
          <div className="card-body">
            {pendingTasks.length > 0 ? (
              <table className="table">
                ...
                <tbody>
                  {pendingTasks.sort((a, b) => new Date(a.due_date) - new Date(b.due_date)).slice(0, 5).map(task => (
                    <tr key={task.id}>
                      <td>{task.title}</td>
                      <td>{new Date(task.due_date).toLocaleDateString()}</td>
                      <td><span className={`badge badge-${task.priority === 'Critical' || task.priority === 'High' ? 'error' : 'warning'}`}>{task.priority}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ) : (
              <div className="empty-state"><p>No pending tasks</p></div>
            )}
          </div>
        </div>
      </div>
...
function StaffClients() {
  ...
  if (selectedClient) {
    const client = assignedClients.find(c => c.id === selectedClient);
    const projects = data.projects.filter(p => p.company_id === selectedClient);
    const tickets = data.supportTickets.filter(t => t.company_id === selectedClient);

    return (
      <div>
        <button className="btn btn-secondary mb-16" onClick={() => setSelectedClient(null)}>← Back to Clients</button>
        <div className="card">
          <div className="card-header">
          ...
          <div className="card-body">
            <div className="form-row mb-24">
              <div>
                <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)' }}>Industry</div>
                <div style={{ fontWeight: 'var(--font-weight-semibold)' }}>{client?.industry}</div>
              </div>
              <div>
            ...
            <h4 style={{ marginBottom: 'var(--space-16)' }}>Projects</h4>
            {projects.length > 0 ? (
              <table className="table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {projects.map(p => (
                    <tr key={p.id}>
                      ...
                      <td><span className={`badge badge-${p.status === 'Completed' ? 'success' : 'info'}`}>{p.status}</span></td>
                      <td>{p.progress}%</td>
                    </tr>
                  ))}
                </tbody>
            ...
            <h4 style={{ marginTop: 'var(--space-24)', marginBottom: 'var(--space-16)' }}>Support Tickets</h4>
            {tickets.length > 0 ? (
              <table className="table">
                <thead>
                  <tr>
                ...
                <tbody>
                  {tickets.map(t => (
                    <tr key={t.id}>
                      <td>{t.title}</td>
                      <td><span className={`badge badge-${t.priority === 'High' ? 'error' : 'warning'}`}>{t.priority}</span></td>
      ...
      <div className="card">
        ...
        <div className="card-body">
          {assignedClients.length > 0 ? (
            <table className="table">
              ...
              <tbody>
                {assignedClients.map(client => {
                  const projects = data.projects.filter(p => p.company_id === client.id);
                  return (
                    <tr key={client.id}>
                      <td>
                      ...
                      <td>{projects.length}</td>
                      <td>
                        <button className="btn btn-secondary btn-small" onClick={() => setSelectedClient(client.id)}>View Details</button>
                      </td>
                    </tr>
                  );
                })}
            ...
            <div className="empty-state">
              <div className="empty-state-icon">🏢</div>
              <div className="empty-state-title">No clients assigned</div>
...
function StaffProjects() {
  ...
  if (selectedProject) {
    ...
    const company = data.companies.find(c => c.id === project?.company_id);
    const projectTasks = data.tasks.filter(t => t.project_id === selectedProject);
      ...
      <div>
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div className="form-row mb-24">
              <div>
                <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)' }}>Client</div>
              ...
              <div>
                <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)' }}>Progress</div>
                <div style={{ fontWeight: 'var(--font-weight-semibold)' }}>{project?.progress}%</div>
              </div>
              <div>
                <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)' }}>Timeline</div>
                <div>{new Date(project?.start_date).toLocaleDateString()} - {new Date(project?.end_date).toLocaleDateString()}</div>
              </div>
            </div>

            <h4 style={{ marginBottom: 'var(--space-16)' }}>Milestones</h4>
            {project?.milestones?.map(milestone => (
              <div key={milestone.id} style={{ padding: 'var(--space-16)', background: 'var(--color-background)', borderRadius: 'var(--radius-base)', marginBottom: 'var(--space-12)', border: '1px solid var(--color-border)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
            ...
            <h4 style={{ marginTop: 'var(--space-24)', marginBottom: 'var(--space-16)' }}>Tasks</h4>
              ...
              <table className="table">
                ...
                <tbody>
                  {projectTasks.map(task => (
                    <tr key={task.id}>
                      <td>{task.title}</td>
                      <td><span className={`badge badge-${task.status === 'Done' ? 'success' : 'info'}`}>{task.status}</span></td>
                      <td><span className={`badge badge-${task.priority === 'Critical' || task.priority === 'High' ? 'error' : 'warning'}`}>{task.priority}</span></td>
                      <td>{new Date(task.due_date).toLocaleDateString()}</td>
                    </tr>
                  ))}
        ...
        {assignedProjects.map(project => {
          ...
          return (
            <div key={project.id} className="stat-card" style={{ cursor: 'pointer' }} onClick={() => setSelectedProject(project.id)}>
              ...
              <div style={{ fontSize: 'var(--font-size-lg)', fontWeight: 'var(--font-weight-semibold)', marginBottom: 'var(--space-8)' }}>{project.name}</div>
              <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)', marginBottom: 'var(--space-8)' }}>{company?.name}</div>
              <div style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-secondary)', marginBottom: 'var(--space-12)' }}>{project.service_type}</div>
              <div style={{ background: 'var(--color-secondary)', height: '8px', borderRadius: '4px', marginBottom: 'var(--space-8)', overflow: 'hidden' }}>
                <div style={{ width: `${project.progress}%`, height: '100%', background: 'var(--color-primary)' }}></div>
              </div>
...
function StaffTasks() {
  const { data, currentUser, updateTask } = useApp();
  const myTasks = data.tasks.filter(t => t.assignee_id === currentUser.id);
  const [showModal, setShowModal] = useState(false);
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="form-group">
                <label className="form-label">Description</label>
                <div style={{ padding: 'var(--space-12)', background: 'var(--color-background)', borderRadius: 'var(--radius-base)' }}>{selectedTask.description}</div>
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">Status</label>
                  <select className="form-input" value={selectedTask.status} onChange={(e) => { updateTask(selectedTask.id, { status: e.target.value }); setShowModal(false); }}>
                    {statuses.map(s => <option key={s}>{s}</option>)}
                  </select>
                </div>
                <div className="form-group">
                  <label className="form-label">Priority</label>
                  <span className={`badge badge-${selectedTask.priority === 'Critical' || selectedTask.priority === 'High' ? 'error' : 'warning'}`}>{selectedTask.priority}</span>
                </div>
              </div>
              <div className="form-group">
                <label className="form-label">Due Date</label>
...
function StaffPerformance() {
  ...
  const myTasks = data.tasks.filter(t => t.assignee_id === currentUser.id);
  const completedTasks = myTasks.filter(t => t.status === 'Done');
  const myProjects = data.projects.filter(p => currentUser.assigned_projects?.includes(p.id) || p.assigned_to_id === currentUser.id);
  const completionRate = myTasks.length > 0 ? ((completedTasks.length / myTasks.length) * 100).toFixed(0) : 0;
  ...
  return (
    <div>
      ...
      <div className="card">
        <div className="card-header">
          <h3 className="card-title">My Task Performance</h3>
        </div>
        <div className="card-body">
          <table className="table">
            <thead>
...
function StaffSupport() {
  const { data, currentUser } = useApp();
  const myTickets = data.supportTickets.filter(t => t.assigned_to_id === currentUser.id || currentUser.assigned_clients?.includes(t.company_id));
  const [selectedTicket, setSelectedTicket] = useState(null);

  if (selectedTicket) {
    const ticket = myTickets.find(t => t.id === selectedTicket);
    const company = data.companies.find(c => c.id === ticket?.company_id);
        ...
        <div className="card-body">
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input className="form-input" value={currentUser.role} disabled />
            </div>
            <div className="form-group">
...
function App() {
  ...
  const renderView = () => {
    if (loginType === 'client') {
      ...
      switch (currentView) {
        ...
        case 'clients': return <StaffClients />;
        case 'projects': return <StaffProjects />;
        case 'tasks': return <StaffTasks />;
        case 'performance': return <StaffPerformance />;
  ...
  const navItems = loginType === 'client' ? clientNavItems : loginType === 'staff' ? staffNavItems : adminNavItems;
    ...
    <div className="app-container">
      <div className="sidebar">
        <div className="sidebar-header">
          ...
          {loginType === 'staff' && <div style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-secondary)', marginTop: 'var(--space-4)' }}>Staff Panel</div>}
        </div>
